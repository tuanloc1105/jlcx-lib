...
public class ControllerProcessor extends AbstractProcessor {
    ...
    private static String extractApiKeyValidationMethod(boolean applicationHaveAPIKey) {
        ...
        if (applicationHaveAPIKey) {
            apiKeyAuthHandler = "\n    public void validateApiKey(io.vertx.ext.web.RoutingContext context) {\n" +
                    ...
                    "        String validApiKey = vn.io.lcx.common.constant.CommonConstant.applicationConfig.getProperty(\"server.api-key\");\n" +
                    "        if (!((apiKey + vn.io.lcx.common.constant.CommonConstant.EMPTY_STRING).equals(validApiKey))) {\n" +
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (!classMap.isEmpty()) {
            ...
            for (Map.Entry<TypeElement, List<ExecutableElement>> currentClass : classMap.entrySet()) {
                ...
                for (ExecutableElement e : currentClass.getValue()) {
                    ...
                    if (StringUtils.isNotBlank(routerConfigureCode)) {
                        ...
                        routerConfigureCode += String.format(
                                "\n" +
                                        "                    .handler(ctx -> this.controller%d.%s(vn.io.lcx.vertx.base.wrapper.RoutingContextLcxWrapper\n" +
                                        "                                    .init(ctx)));",
                                count,
            ...
            try {
                JavaFileObject builderFile = this.processingEnv.getFiler().createSourceFile("vn.io.lcx.vertx.verticle.ApplicationVerticle");
