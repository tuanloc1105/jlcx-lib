...
@SupportedAnnotationTypes("vn.io.lcx.jpa.annotation.Service")
public class ServiceProcessor extends AbstractProcessor {
    ...
    private void generateCode(ProcessorClassInfo processorClassInfo) throws Exception {
        ...
        processorClassInfo.getMethods().forEach((method, executableElement) -> {
            ...
            codeLines.add(
                    "final boolean isRoot = vn.io.lcx.jpa.context.JpaContext.isJpaContextEmpty();"
            );
            codeLines.add(
                    "if (isRoot) {"
            );
            codeLines.add(
                    "    vn.io.lcx.jpa.context.JpaContext.put(vn.io.lcx.common.utils.RandomUtils.generateRandomString(6), vn.io.lcx.common.utils.RandomUtils.getRandomNumber(0, 100));"
            );
            codeLines.add(
                    "}"
            );
            ...
            if (transactionalAnnotation != null) {
                codeLines.add(
                        "vn.io.lcx.jpa.context.JpaContext.setTransactionOpen(true);"
                );
                codeLines.add(
                        String.format(
                                "vn.io.lcx.jpa.context.JpaContext.setTransactionIsolation(%d);",
                                transactionalAnnotation.isolation()
                        )
                );
                codeLines.add(
                        String.format(
                                "vn.io.lcx.jpa.context.JpaContext.setTransactionMode(%d);",
                                transactionalAnnotation.mode()
                        )
                );
                for (AnnotationMirror annotationMirror : executableElement.getAnnotationMirrors()) {
                    if (annotationMirror.getAnnotationType().toString().equals("vn.io.lcx.jpa.annotation.Transactional")) {
            ...
            codeLines.add(
                    "        vn.io.lcx.jpa.context.JpaContext.commit();"
            );
            codeLines.add(
                    "    }"
            );
            ...
            exceptionClassesToRollback.forEach(
                    exceptionClass -> {
                        ...
                        codeLines.add("        vn.io.lcx.jpa.context.JpaContext.rollback();");
                        codeLines.add("    }");
                        if (!"vn.io.lcx.vertx.base.exception.InternalServiceException".equals(exceptionClass)) {
            ...
            if (!exceptionClassesToRollback.contains("java.lang.Exception")) {
                ...
                codeLines.add("    if (e instanceof vn.io.lcx.vertx.base.exception.InternalServiceException) {");
                codeLines.add("        throw e;");
                codeLines.add("    }");
                codeLines.add("    throw new java.lang.RuntimeException(e);");
                codeLines.add("}");
            ...
            codeLines.add(
                    "        vn.io.lcx.jpa.context.JpaContext.close();"
            );
            codeLines.add(
                    "        vn.io.lcx.jpa.context.JpaContext.clearAll();"
            );
            codeLines.add(
                    "    }"
            );
