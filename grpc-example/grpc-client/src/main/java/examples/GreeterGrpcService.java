...
public class GreeterGrpcService extends GreeterService implements Service {
  ...
  public static final ServiceMethod<examples.HelloRequest, examples.HelloReply> SayHello = ServiceMethod.server(
    ...
    GrpcMessageDecoder.decoder(examples.HelloRequest.newBuilder()));
    ...
    private class Invoker implements Service {
      ...
      private <Req, Resp> Handler<io.vertx.grpc.server.GrpcServerRequest<Req, Resp>> resolveHandler(ServiceMethod<Req, Resp> serviceMethod) {
        if (SayHello == serviceMethod) {
          Handler<io.vertx.grpc.server.GrpcServerRequest<examples.HelloRequest, examples.HelloReply>> handler = this::handle_sayHello;
